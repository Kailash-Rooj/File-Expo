...
| 🗑️ **Trash** | Soft-delete files into a Trash bin. Restore or permanently delete them later |
...
| `POST` | `/trash/move/{subpath}` | Move item to trash |
| `GET` | `/trash/list` | List all items in trash |
| `POST` | `/trash/restore/{id}` | Restore item from trash |
| `DELETE` | `/trash/delete/{id}`| Permanently delete item |
| `DELETE` | `/trash/empty`    | Empty the trash bin |
